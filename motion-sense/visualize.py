...
from model_utils import MotionSenseDataset, ASFDCLClassifier, seed_worker
try:
...
def measure_efficiency(model, device, input_shape=(1, 128, 6), warmup=10, iters=100):
    """
...
def main():
    ...
    MODEL_PATH = '/content/drive/MyDrive/Colab Notebooks/ASF/motion-sense/motion-sense_ASF.pth'
    ...
    full_dataset = MotionSenseDataset(
        DATA_PATH, window_size=128, step_size=64, normalize=False
    )
    ...
    print(f"Total samples: {len(full_dataset)} | Test samples: {len(test_subset)}")
